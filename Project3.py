...
Clean implementation using only numpy, matplotlib, and scipy
"""
...
def calculate_observables(magnetization, energy, T, L):
    ...
    tau_m = autocorrelation_time(magnetization)
    tau_e = autocorrelation_time(energy)
    
    # Effective sample sizes
    n_eff_m = len(magnetization) / tau_m
    n_eff_e = len(energy) / tau_e
    
    # Error estimates
    m_error = np.std(magnetization) / np.sqrt(n_eff_m)
    e_error = np.std(energy) / np.sqrt(n_eff_e)
...
def temperature_scan(L=32, T_min=1.5, T_max=3.5, n_temps=15):
    ...
    # Optional: If you want to ensure Tc is included, replace nearest point
    # Find closest temperature to Tc and replace it with exact Tc
    ...
    for T in temps:
        ...
        print(f"  Metropolis: |m| = {metro_obs['magnetization']:.3f}, τ = {metro_obs['tau_magnetization']:.1f}")
        print(f"  Wolff:      |m| = {wolff_obs['magnetization']:.3f}, τ = {wolff_obs['tau_magnetization']:.1f}")
